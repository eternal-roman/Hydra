...
| `HYDRA_BUY_OFFSET_DISABLED` | execution | `=1` disables the regime-gated BUY limit offset (BUYs revert to raw bid). Default off (offset active). Table lives in `hydra_agent.py:_BUY_LIMIT_OFFSET_BPS` keyed by `(base, quote_class, regime)`; only `VOLATILE` and `TREND_DOWN` carry offsets, `RANGING`/`TREND_UP` stay at raw bid to avoid missing fills in chop/rallies. SOL on stable quote gets the largest offset (90 bps in TREND_DOWN) — derived from empirical post-fill drawdown analysis (median 1h DD −0.63% vs BTC/USD −0.33%). |
